...
class UAVBodyActionAutoYawCfg(BodyActionCfg):
    ...
    姿态模拟 (simulate_attitude):
    - True (方案四): 根据加速度模拟 roll/pitch 倾斜
    - False (方案五): 纯质点模型，始终保持水平
    ...
    # 姿态模拟开关
    simulate_attitude: bool = True  # True=方案四(模拟倾斜), False=方案五(纯质点)
    ...
    acc_up: float = 3.0     # 向上最大加速度 (m/s²)
    acc_down: float = 2.0   # 向下最大加速度 (m/s²)
    yaw_acc: float = 2.0    # 角加速度限制 (rad/s²)
